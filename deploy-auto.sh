#!/bin/bash
# ============================================
# SCRIPT DE DESPLIEGUE AUTOMÁTICO - NO INTERACTIVO
# ============================================
# Uso: ./deploy-auto.sh [version] [descripcion]
# Ejemplo: ./deploy-auto.sh 2.1.0 "Fix de bugs"
# Si no se especifica versión, incrementa el patch automáticamente
# Si no se especifica descripción, usa una por defecto
...
echo -e "${PURPLE}"
echo "╔════════════════════════════════════════════════════════════════════╗"
echo "║       DESPLIEGUE AUTOMÁTICO - VICTOR GUZMAN POS                    ║"
echo "╚════════════════════════════════════════════════════════════════════╝"
echo -e "${NC}"
...
echo -e "${CYAN}📝 Descripción: ${YELLOW}$DESCRIPTION${NC}"
...
sed -i '' "s/version: $CURRENT_VERSION+$BUILD_NUMBER/version: $NEW_VERSION+$NEW_BUILD_NUMBER/" "$PUBSPEC_FILE"
echo -e "   ${GREEN}✓${NC} pubspec.yaml: $NEW_VERSION+$NEW_BUILD_NUMBER"
...
if [ "$BUILD_VERSION" = "$NEW_VERSION" ]; then
    ...
    echo -e "   ${RED}✗${NC} Error: Build tiene versión incorrecta ($BUILD_VERSION vs $NEW_VERSION)"
    exit 1
fi
...
SERVER_VERSION=$(sshpass -p "$SERVER_PASS" ssh -o StrictHostKeyChecking=no "$SERVER_USER@$SERVER_IP" \
    "grep -o \"REQUIRED_VERSION = '[^']*'\" $REMOTE_PATH/index.html | cut -d\"'\" -f2" 2>/dev/null)
...
echo "╚════════════════════════════════════════════════════════════════════╝"
echo -e "${NC}"
echo -e "${GREEN}Versión anterior: ${YELLOW}$CURRENT_VERSION${NC}"
echo -e "${GREEN}Versión desplegada: ${YELLOW}$NEW_VERSION${NC}"
echo -e "${GREEN}Build: ${YELLOW}$BUILD_DATE${NC}"
echo -e "${GREEN}Descripción: ${YELLOW}$DESCRIPTION${NC}"
