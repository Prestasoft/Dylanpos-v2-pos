      ...
      child: Consumer(
        builder: (_, ref, watch) {
          ...
          return dueReport.when(data: (dueReport) {
            ...
            final filterData = invoiceNumber.isEmptyOrNull ? dueReport.where((element) => element.customerPhone == selectedUserId && element.dueAmount! > 0.0).toList() : dueReport.where((element) => element.invoiceNumber == invoiceNumber).toList();
                                          ...
                                          child: DataTable(
                                            ...
                                            rows: List.generate(
                                              ...
                                              (ind) => DataRow(cells: [
                                                ...
                                                DataCell(
                                                  Text('$globalCurrency ${dueTransactionModel?.productList?[ind].subTotal.toString() ?? ''}', style: kTextStyle.copyWith(color: kTitleColor)),
                                                ),
                                                DataCell(
                                                  Text(dueTransactionModel?.productList?[ind].quantity.toString() ?? '', style: kTextStyle.copyWith(color: kTitleColor)),
                                                ),
                                                DataCell(
                                                  Text('$globalCurrency 2800.00', style: kTextStyle.copyWith(color: kTitleColor)),
                                                ),
                                              ]),
                                            ),
                                          ),
