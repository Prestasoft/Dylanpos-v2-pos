...
class AdditionalClothesReservationScreen extends ConsumerStatefulWidget {
  const AdditionalClothesReservationScreen({super.key});

  @override
  // ignore: library_private_types_in_public_apiS, library_private_types_in_public_api
  _AdditionalClothesReservationScreen createState() => _AdditionalClothesReservationScreen();
}

class _AdditionalClothesReservationScreen extends ConsumerState<AdditionalClothesReservationScreen> {
  ...
  late String _packagePrice;
  late double _packagePriceValue;
  ...
  void didChangeDependencies() {
    ...
    if (!_initialized) {
      ...
      _packageId = extra?['packageId'] ?? '';
      _packageName = extra?['packageName'] ?? '';
      _reservationId = extra?['reservationId'] ?? '';
      _selectedDate = extra?['selectedDate'] as DateTime?;
      _selectedTime = extra?['selectedTime'] as TimeOfDay?;
      _clientId = extra?['clientId'] ?? '';
      _clientName = extra?['clientName'] ?? '';
      _packagePrice = extra?['package_price'] ?? '';
      _packagePriceValue = double.tryParse(_packagePrice) ?? 0.0;
    ...
    for (int i = 0; i < dressReservations.length; i++) {
      ...
      final isAvailable = await ref.read(
        isClothesAvailableForRangeProvider({
          ...
          'startDate': _formatDate(_selectedDate ?? DateTime.now()),
          'isAdditional': true,
        }).future,
      );
  ...
  List<Widget> TextSummaryPrice(BuildContext context, TextTheme _textTheme, ColorScheme _colors) {
    return [
      ...
      _buildHeader(context, "Adicionar Vestimenta"),
      SizedBox(height: 24),
      // Fecha de la reserva
      Card(
        child: ListTile(
      ...
      Card(
        child: ListTile(
          ...
          subtitle: Text(
            _packageName + (_packagePrice.isNotEmpty ? ' (Precio: \$ $_packagePrice)' : ''),
            style: TextStyle(
              color: Colors.black,
            ),
          ),
  ...
  Widget _buildTotalPrice(dynamic valueSelected) {
    double totalPrice = _packagePriceValue + selectedValues.values.where((e) => e['vestidoPrice'] != null).map((e) => double.tryParse(e['vestidoPrice'].toString()) ?? 0.0).fold(0.0, (a, b) => a + b);
    ...
    return Text(
      "Confirmar Reserva + Adicionales. Total: (${formatCurrency.format(totalPrice)})",
  ...
  Future<AvailabilityResult> _buildAvailableClothRetry(
    ...
    try {
      ...
      final clothReservation = ClothReservation(
        packageId: a.firstWhere((e) => e.name == "Adicionales de Reserva").id,
        packageName: a.firstWhere((e) => e.name == "Adicionales de Reserva").name,
        dressId: '',
        dressName: '',
        branchId: '',
      ...
      child: ElevatedButton(
        ...
        onPressed: isConfirmButtonEnabled
            ? () async {
                ...
                // Agregar Adicionales a Reserva

                var a = ref.read(servicePackagesProvider.notifier).searchPackages("Adicionales de Reserva");

                ClothReservation clothReservation = ClothReservation(
                  packageId: a.firstWhere((element) => element.name == "Adicionales de Reserva").id,
                  packageName: a.firstWhere((element) => element.name == "Adicionales de Reserva").name,
                  dressId: '',
                  dressName: '',
                  branchId: '',
                  dressReservations: dressReservations,
