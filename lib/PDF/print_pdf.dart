...
class GeneratePdfAndPrint {
  ...
  Future<void> uploadSaleInvoice(
    ...
    var pdfData =
        await generateSaleDocument(personalInformation: personalInformationModel, transactions: saleTransactionModel, generalSetting: setting.companyName as GeneralSettingModel);
  ...
  Future<void> uploadSaleQuoteInvoice(
      {required PersonalInformationModel personalInformationModel, required SaleTransactionModel saleTransactionModel, bool? fromInventorySale}) async {
    var data = await currentSubscriptionPlanRepo.getCurrentSubscriptionPlans();
    if (data.whatsappMarketingEnabled && (saleTransactionModel.sendWhatsappMessage ?? false)) {
      try {
        EasyLoading.show(status: 'Sending message...', dismissOnTap: true);
  ...
  Future<void> uploadPurchaseInvoice(
      {required PersonalInformationModel personalInformationModel,
  ...
  Future<void> uploadPurchaseReturnInvoice(
      {required PersonalInformationModel personalInformationModel,
      required PurchaseTransactionModel purchaseTransactionModel,
      bool? fromInventorySale,
      required GeneralSettingModel setting}) async {
  ...
  //       if ((fromInventorySale ?? false) && context.mounted) {
  ...
  Future<void> printQuotationInvoice(
      ...
      required SaleTransactionModel saleTransactionModel,
      BuildContext? context,
      bool? isFromInventorySale,
      bool? isFromQuotation}) async {
    var data = await currentSubscriptionPlanRepo.getCurrentSubscriptionPlans();
    if (data.whatsappMarketingEnabled && (saleTransactionModel.sendWhatsappMessage ?? false)) {
      try {
        EasyLoading.show(status: 'Sending message...', dismissOnTap: true);
  ...
  Future<void> printDueInvoice(
    ...
    EasyLoading.show(status: 'Generating PDF...', dismissOnTap: true);
    var pdfData = await GeneratePdfAndPrint().generateDueDocument(personalInformation: personalInformationModel, transactions: dueTransactionModel, setting: setting);
    await uploadPdfToFirebase(pdfData, 'due', dueTransactionModel.invoiceNumber);
    EasyLoading.dismiss();
    await Printing.layoutPdf(
      dynamicLayout: true,
  ...
  FutureOr<Uint8List> generateDueDocumentStyle2(
      {required DueTransactionModel transactions, required PersonalInformationModel personalInformation, required GeneralSettingModel generalSetting}) async {
    final pw.Document doc = pw.Document();
  ...
  FutureOr<Uint8List> generateDueDocument(
      {required DueTransactionModel transactions, required PersonalInformationModel personalInformation, required GeneralSettingModel setting}) async {
    final pw.Document doc = pw.Document();
    final netImage = await networkImage(
      'https://www.nfet.net/nfet.jpg',
    );
      ...
      pw.MultiPage(
        ...
        footer: (pw.Context context) {
          return pw.Column(children: [
            ...
            pw.SizedBox(height: 5),
            pw.Text('Powered By ${setting.companyName.isNotEmpty == true ? setting.companyName : pdfFooter}', style: const pw.TextStyle(fontSize: 10, color: PdfColors.black)),
    ...
    pw.MultiPage(
      ...
      footer: (pw.Context context) {
        return pw.Column(children: [
          ...
          pw.Text('Powered By ${generalSetting.companyName.isNotEmpty == true ? generalSetting.companyName : pdfFooter}',
