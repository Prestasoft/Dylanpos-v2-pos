...
class GeneratePdfAndPrint {
  Future<void> uploadPdfToFirebase(
    ...
    Reference ref = storage
        .ref()
        .child('$constUserId/$fileType/invoice-$invoiceNumber.pdf');
  ...
  Future<void> uploadSaleReturnInvoice({
    ...
    //Convert unint8List to pdf and upload in to firebase storage
    await uploadPdfToFirebase(
        pdfData, 'salereturn', saleTransactionModel.invoiceNumber);
    EasyLoading.dismiss();
  }
  ...
  Future<void> uploadSaleQuoteInvoice(
    ...
    EasyLoading.show(status: 'Generating PDF...', dismissOnTap: true);
    var pdfData = await generateQuotationDocument(
        personalInformation: personalInformationModel,
        transactions: saleTransactionModel);
    //Convert unint8List to pdf and upload in to firebase storage
    await uploadPdfToFirebase(
        pdfData, 'salequote', saleTransactionModel.invoiceNumber);
    EasyLoading.dismiss();
  }
  ...
  Future<void> uploadPurchaseInvoice(
    ...
    var data = await currentSubscriptionPlanRepo.getCurrentSubscriptionPlans();
    if (data.whatsappMarketingEnabled &&
        (purchaseTransactionModel.sendWhatsappMessage ?? false)) {
      try {
        EasyLoading.show(status: 'Sending message...', dismissOnTap: true);
  ...
  Future<void> uploadPurchaseReturnInvoice(
    ...
    //Convert unint8List to pdf and upload in to firebase storage
    await uploadPdfToFirebase(
        pdfData, 'purchasereturn', purchaseTransactionModel.invoiceNumber);
    EasyLoading.dismiss();
  }
  ...
  Future<void> uploadDueInvoice(
    ...
    var data = await currentSubscriptionPlanRepo.getCurrentSubscriptionPlans();
    if (data.whatsappMarketingEnabled &&
        (dueTransactionModel.sendWhatsappMessage ?? false)) {
      try {
        EasyLoading.show(status: 'Sending message...', dismissOnTap: true);
  ...
  Future<void> printQuotationInvoice(
      ...
      bool? isFromQuotation,
      String printFormat = 'large'}) async {
    var data = await currentSubscriptionPlanRepo.getCurrentSubscriptionPlans();
    if (data.whatsappMarketingEnabled &&
        (saleTransactionModel.sendWhatsappMessage ?? false)) {
      try {
